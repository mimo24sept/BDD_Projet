...
- **Données** : `BDD/Projet_BDD.sql` (tables `User`, `Role`, `Materiel`, `Categorie`, `Emprunt`, `Rendu`, `Notification`, `Prolongation`). Créations lazy : `MaintenanceRequest`, `ReservationRequest`, colonne `User.LastLogin`, colonne `User.ForcePasswordReset`, colonne `Materiel.Image`.
...
- Actions <span style="color:#d9534f;font-weight:600;">admin uniquement</span> : création/suppression/modification matériel, reset MDP utilisateurs, rendus/annulations directes, stats globales, comptes. Maintenance : administrateur ou technicien ; si une maintenance technicien chevauche des réservations, elle part en demande “en attente” pour validation admin (sans suppression tant que non validée).
- Réinitialisation MDP : l’admin génère un mot de passe temporaire ; l’utilisateur est forcé de le changer à la première connexion après reset (via le menu “Modifier mdp” en haut à droite).
...
8) **Mots de passe** : reset admin (`POST /api/auth.php?action=reset_password`) → mot de passe temporaire ; login utilisateur forcé à changer le mot de passe via le panneau “Modifier mdp” (top-right).
9) **Edition matériel** : admin ouvre la modale d’édition, modifie lieu/état/image/catégories (`POST /api/equipment.php?action=update`).
...
- **assets/app.js** : point d’entrée, branche les listeners, charge session + données, orchestre modales réservation/maintenance/édition + panneau de changement de MDP.
- **assets/app/api.js** : appels fetch et normalisation des réponses (dont update matériel et reset/changement de MDP).
- **assets/app/render.js** : rendu UI (catalogue, prêts, maintenance, comptes, stats) + export PDF.
- **assets/app/calendar.js** : sélection des dates, blocage périodes, logique de modale (réservation/maintenance/édition).
...
- **assets/login.js** : bascule login/register, bouton œil, loader ripple, `apiLogin`/`apiRegister` + mémorisation MDP temporaire.
- **api/auth.php** : sessions, rôles, LastLogin, reset/changement MDP, CRUD users (admin).
- **api/equipment.php** : catalogue + périodes actives, réservations (refus passé/conflits), maintenance (ajustements + demandes), CRUD matériel (admin, dont update).
...
- **Frontend (`assets/app.js`)**
  ...
  - Edition/MDP: valide la modale d’édition matériel et force le changement de mot de passe si requis.
- **Frontend (`assets/app/api.js`)**
  ...
  - `apiFetchUsers`, `apiSetUserRole`, `apiDeleteUser`, `apiResetPassword`, `apiChangePassword` : gestion des comptes + MDP.
...
- **Frontend (`assets/app/calendar.js`)**
  - `openModal`, `openExtendModal`, `openEditModal`, `closeModal` : gestion de la modale.
...
- **Frontend (`assets/login.js`)**
  ...
  - `updateSecretVisibility`, `switchMode`, `apiLogin`, `apiRegister` (+ stockage mdp temporaire).
- **Backend Auth (`api/auth.php`)**
  ...
  - `change_password` / `reset_password` : changement perso ou reset admin avec MDP temporaire + flag de reset forcé.
...
- **Backend Catalogue/Reservations (`api/equipment.php`)**
  ...
  - `create_equipment` / `update_equipment` / `delete_equipment` : CRUD admin, upload image, renvoi item pour rafraîchir l’UI.
...
- Reset MDP : admin génère un mot de passe temporaire, login utilisateur → changement forcé du mot de passe.
- Edition matériel : modifier lieu/état/image d’un matériel et vérifier la mise à jour dans le catalogue.
